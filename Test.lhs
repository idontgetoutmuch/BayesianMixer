...
posteriorizeA q d = do
  ...
  return $ H.bmap (\(u, v) p -> p * likelihoodA (M.vector [u, v]) d) newQ
\end{code}
...
where $x_k^{(i)} \sim p(x_k \, \vert \, x_k^{(i-1)})$
...
lSaxis :: [(Double, Double)] -> Axis B V2 Double
lSaxis xs = r2Axis &~ do
  linePlot' xs
  xMin .= Just (-5.0)
  xMax .= Just 5.0
...
main :: IO ()
main = do
  renderRasterific "diagrams/prior.png"
                   (dims2D 500.0 500.0)
                   (renderAxis $ lSaxis $ H.asList (ns!!0))
  renderRasterific "diagrams/qs1.png"
                   (dims2D 500.0 500.0)
                   (renderAxis $ lSaxis $ H.asList (ns!!1))
  renderRasterific "diagrams/qsN.png"
                   (dims2D 500.0 500.0)
                   (renderAxis $ lSaxis $ H.asList (ns!!9))
